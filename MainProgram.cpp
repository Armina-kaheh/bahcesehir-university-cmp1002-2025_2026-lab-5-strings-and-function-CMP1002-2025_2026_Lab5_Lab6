...
 
// ---- String Manipulation ----
 MyString MyString::toUpperCase() const {     // TODO: Return a NEW MyString with all characters converted to upper case     // Hint: Use std::transform with ::toupper     // Do NOT modify the original object     string result=data;     transform(result.begin(),result.end(),result.begin(),[](unsigned char c){return toupper(c);});     return MyString(result); }
 
MyString MyString::toLowerCase() const {     // TODO: Return a NEW MyString with all characters converted to lower case     // Hint: Use std::transform with ::tolower     // Do NOT modify the original object     string result=data;     transform(result.begin(),result.end(),result.begin(),[](unsigned char c){return tolower(c);});     return MyString(result);  }
 
MyString MyString::trim() const {     // TODO: Return a NEW MyString with leading and trailing whitespace removed     // Whitespace includes: space, tab (\t), newline (\n), carriage return (\r)     // Hint: Use find_first_not_of and find_last_not_of     size_t start=data.find_first_not_of(" \t\n\r");     if(start==string::npos)return MyString("");       size_t end=data.find_last_not_of(" \t\n\r");       return MyString(data.substr(start,end-start+1)); }
 
MyString MyString::reverse() const {     // TODO: Return a NEW MyString with characters in reverse order     // Hint: Use std::reverse on a copy       string result=data;       std::reverse(result.begin(),result.end());       return MyString(result); }
 
// ---- Search Operations ----
 
int MyString::find(const MyString& target) const {     // TODO: Return index of first occurrence of target, or -1 if not found     // Hint: Use std::string::find, check against string::npos     size_t pos=data.find(target.data);     if(pos== string::npos) return-1;     return static_cast<int> (pos); }
 
int MyString::count(char ch) const {     // TODO: Return the number of occurrences of character ch in the string     int cnt=0;     for(char c: data){         if(c==ch) cnt ++;     }     return cnt; }
 
// ---- Function Overloading: append ---- // These four functions demonstrate FUNCTION OVERLOADING: // Same name "append", different parameter types.
 
MyString MyString::append(const MyString& other) const {     // TODO: Append another MyString to this one and return the result     return MyString(data + other.data); }
 
MyString MyString::append(const char* cstr) const {     // TODO: Append a C-string to this one and return the result     return MyString(data + string(cstr));
 
}
 
MyString MyString::append(char ch) const {     // TODO: Append a single character and return the result     return MyString(data + ch); }
 
MyString MyString::append(int number) const {     // TODO: Append the string representation of an integer and return the result      // Hint: Use std::to_string()     return MyString(data + to_string(number)); }
 
// ---- Function Overloading: replace ---- // These two functions demonstrate FUNCTION OVERLOADING: // Same name "replace", different parameter types.
 
MyString MyString::replace(char oldCh, char newCh) const {     // TODO: Replace ALL occurrences of oldCh with newCh, return new MyString     string result = data;     for (char& c : result) {         if (c == oldCh) c = newCh;     }     return MyString(result); }
 
MyString MyString::replace(const string& oldStr, const string& newStr) const {     // TODO: Replace ALL occurrences of oldStr with newStr, return new MyString     // WARNING: Be careful not to create an infinite loop!     //          After each replacement, advance position past the new string.     string result = data;     size_t pos = 0;     while ((pos = result.find(oldStr, pos)) != string::npos) {         result.replace(pos, oldStr.length(), newStr);         pos += newStr.length();     }     return MyString(result);  }
 
// ---- Operator Overloading ----
 
MyString MyString::operator+(const MyString& other) const {     // TODO: Concatenate two MyString objects and return the result     return MyString(data + other.data); }
